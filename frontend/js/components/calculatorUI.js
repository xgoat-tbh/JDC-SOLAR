...
export class CalculatorUIController {
  ...
  update() {
    ...
    if (this.outSystemSize) this.outSystemSize.innerHTML = `${outputs.systemSize.toFixed(1)} <small class="metric-unit" style="font-size: 1.1rem; color: var(--color-brand-accent-text, #9A3412);">kWp</small>`;
...
export function initCalculator() {
  ...
  calcRoots.forEach(root => {
    if (root.dataset.calcInit === 'true') return;
    root.dataset.calcInit = 'true';
    new CalculatorUIController(root);
  });
}
