...
function initCircuitRoadmapAnimation() {
  ...
  const updateRoadmap = () => {
    ...
    // 1. Batch all geometric reads first (Zero layout invalidation)
    const measurements = Array.from(steps).map((step, idx) => {
      if (stepProgress[idx] >= 1) return null;
      return step.getBoundingClientRect();
    });

    let allCompleted = true;

    // 2. Batch all DOM and style mutations (Zero forced reflow)
    steps.forEach((step, idx) => {
      if (stepProgress[idx] >= 1) return;

      const rect = measurements[idx];
      if (!rect) return;
